...
best_model_path = os.path.dirname(os.path.realpath(__file__)) + '/'

for modelname in os.listdir(best_model_path):
    if "meta" in modelname:
        best_model = modelname[:-5]
...
def Model_Finaltest(sess, TEST_X, filter_size, filter_num, if3d, model, args, load_episode, best_model_path):
    ...
    ossz = list(tuple(TEST_Z.reshape([np.shape(TEST_Z)[0]])))
    return ossz


# def end: Model_Finaltest

def predict(szekvenc):
    # loop end: TEST_NUM
    ...
    graph = tf.Graph()
    with tf.Session(config=conf, graph=graph) as sess:
        ...
        TEST_X = preprocess_seq(szekvenc)

        vissza = Model_Finaltest(sess, TEST_X, filter_size, filter_num, if3d, model, args, load_episode,
                                 best_model_path)
